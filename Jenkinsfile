        ...
        stage('Deploy on other linux machine') {
            steps {
                ...
                withCredentials([
                    ...
                    sh 'sshpass -p ${DEPLOY_PASSWORD} ssh -t -o StrictHostKeyChecking=no ${DEPLOY_USER}@${DEPLOY_MACHINE}'
                    sh 'echo $DOCKERHUB_CREDENTIALS_PSW | docker login -u $DOCKERHUB_CREDENTIALS_USR --password-stdin'
                    sh 'docker stop $(docker ps -q --filter "ancestor=${DOCKERHUB_CREDENTIALS_USR}/${IMAGE_TAG}:latest") || true'
                    sh 'docker rm $(docker ps -a -q --filter "ancestor=${DOCKERHUB_CREDENTIALS_USR}/${IMAGE_TAG}:latest") || true'
                    sh 'docker rmi ${DOCKERHUB_CREDENTIALS_USR}/${IMAGE_TAG}:latest || true'
                    sh 'docker pull ${DOCKERHUB_CREDENTIALS_USR}/${IMAGE_TAG}:latest'
                    sh 'docker run -d -p 9090:9090 ${DOCKERHUB_CREDENTIALS_USR}/${IMAGE_TAG}:latest'
                    sh 'sleep 30'
                    sh 'curl http://${DEPLOY_MACHINE}:9090'
